...
var _buynowModal = _interopRequireDefault(__webpack_require__(13));

var _qaControl = _interopRequireDefault(__webpack_require__(14));

var _serviceCenter = _interopRequireDefault(__webpack_require__(15));
...
$(document).ready(function () {
  ...
  var search = new _search.default(); // Product Listing And Buynow modal control

  if ($('#product-page').length > 0) {
    var buynowModal = new _buynowModal.default();
  } // QA layout management


  if ($('#qa-document').length > 0) {
    var qaControl = new _qaControl.default();
...
function () {
  ...
  _createClass(Home, [{
    ...
    value: function DoSearching() {
      ...
      if (searchTerm) {
        console.log("".concat(realmeVietnamVariables.products_api, "?term=").concat(searchTerm));
        fetch("".concat(realmeVietnamVariables.products_api, "?term=").concat(searchTerm)).then(function (response) {
          ...
          if (data.length > 1) {
            ...
            var resultArrayHTML = data.map(function (value, index) {
              return "<li class=\"result-item\">\n                                <a class=\"link-wrapper\" href=\"".concat(value.landingpage, "\">\n                                    <div class=\"phone-img\">\n                                        <img src=\"").concat(value.thumbnail, "\">\n                                    </div>\n                                    <p class=\"phone-name\">\n                                        ").concat(value.title, "\n                                    </p>\n                                </a>\n                            </li>");
...
function () {
  ...
  function BuynowModal() {
    _classCallCheck(this, BuynowModal);
  ...
  _createClass(BuynowModal, [{
    ...
    key: "SetupModalToggle",
    value: function SetupModalToggle() {
      var _this = this;

      this.$closeModalBtn.on('click', function () {
        var $activeModal = $('.custom-modal.modal-preorder.active');
      ...
      this.$openModalBtn.on('click', function (e) {
        var dataTarget = $(e.target).data('modal-target');
        var $modalTarget = $('#' + dataTarget);
...
function () {
   ...
   * =================================== */
  function QAControl() {
    _classCallCheck(this, QAControl);
  ...
  _createClass(QAControl, [{
    ...
    key: "SetupCategorySelect",
    value: function SetupCategorySelect() {
      this.$CategoryOptions.on('click', function (e) {
        if (!$(e.target).hasClass('active')) {
          console.log($(e.target).data('target')); // Select Category

          $('#qa-document .category-block .category-item.active').removeClass('active');
          $(e.target).addClass('active'); // Show Question Block

          $('.question-answer-list.active').removeClass('active');
          var TargetQuestionGroup = $('.question-answer-list#' + $(e.target).data('target'));
          TargetQuestionGroup.addClass('active');
        }
      });
    }
  }, {
    key: "SetupQuestionToggler",
    value: function SetupQuestionToggler() {
      this.$QuestionTogglers.on('click', function (e) {
        var $targetQuestion = $(e.target).parents('.question-item');
        var $activeQuestion = $('.question-item.active');
        ...
        if ($targetQuestion.hasClass('active')) {
          ...
          $activeQuestion.find('.answer').slideUp('fast'); // Show Selected Question
...
var _axios = _interopRequireDefault(__webpack_require__(16));
  ...
  _createClass(ServiceCenter, [{
    ...
    value: function bindEvents() {
      ...
      this.$selectBox.on('change', function (e) {
        var provinceName = $('option[value=' + $(e.target).val() + ']').html();
        console.log(provinceName);
        console.log("".concat(_this.CENTER_API, "?provinceName=").concat(provinceName.replace(/ /g, '%20'), "&cityName=").concat(provinceName.replace(/ /g, '%20'), "&page=1&limit=10"));

        _axios.default.get("".concat(_this.CENTER_API, "?provinceName=").concat(provinceName.replace(/ /g, '%20'), "&cityName=").concat(provinceName.replace(/ /g, '%20'), "&page=1&limit=10"), {
          mode: 'no-cors'
        }).then(function (data) {
